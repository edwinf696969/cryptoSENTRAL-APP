...
def get_simple_vader_score(text: str, lexicon: dict) -> float:
    ...
    for i, word in enumerate(words):
        ...
        if i > 0 and words[i-1] in ['not', 'no', 'never', 'without']:
            word_score *= -0.74
        score += word_score
    if score != 0: score = score / np.sqrt((score * score) + 15)
    return score
...
def load_data():
    ...
    if not structured_path.exists() or not unstructured_path.exists():
        st.error("Data files not found. Please ensure they are in your GitHub repository.")
        st.stop()
...
def run_sentiment_pipeline(unstructured_df):
    ...
    # FIX: Ensure the date column is properly parsed before setting as index
